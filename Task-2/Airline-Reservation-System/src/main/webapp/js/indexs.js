...
function populate_flights_table(){
    query('flight', 'get', null, 'application/json', true, function(res){
        if(res[0] === 200){
...
function add_flight_btn_clicked(){
	let trip = document.querySelector('input[name=flight-type]:checked').value;
	let airline_id = document.getElementById('airline-id-input').value;
	let airline_name = document.getElementById('airline-name-input').value;
	let from = document.getElementById('from-input').value;
	let to = document.getElementById('to-input').value;
	let dep_date = document.getElementById('dep-date-input').value;
	let dep_time = document.getElementById('dep-time-input').value;
	let ret_date = document.getElementById('ret-date-input').value;
	let ret_time = document.getElementById('ret-time-input').value;
	let first_seats = document.getElementById('first-seats-input').value;
	let biz_seats = document.getElementById('biz-seats-input').value;
	let eco_seats = document.getElementById('eco-seats-input').value;
	let duration = document.getElementById('duration-input').value;

    query('flight', 'post', {"airlineId": airline_id, "airlineName": airline_name, "trip": trip, "from": from, "to": to, "depDate": dep_date, "depTime": dep_time, "retDate": ret_date, "retTime": ret_time, "firstSeats": first_seats, "bizSeats": biz_seats, "ecoSeats": eco_seats, "duration": duration}, 'application/json', false, function(res){
        if(res[0] === 200){
        	console.log("Flight Added!");
        }
        else if(res[0] === 404){
